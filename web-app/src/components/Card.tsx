...
function togglePokemon(
  list: Pokemon[] | null | undefined,
  item: Pokemon
): Pokemon[] {
  if (!list) return [item];
  const exists = list.some((p) => p.id === item.id);
  return exists ? list.filter((p) => p.id !== item.id) : [...list, item];
}

export const MiniCard: React.FC<CardProps> = ({ pokemon }) => {
  return (
    <div className="bg-amber-300 rounded-2xl p-2 h-[40px] flex items-center">
...
const Card: React.FC<CardProps> = ({ pokemon, setSelectedPokemon }) => {
  ...
  return (
    <div
      className={` bg-gradient-to-r from-amber-300 via-amber-100 to-amber-300  p-1 rounded-2xl w-[250px] shadow-md outline-orange-400 hover:outline-dashed`}
      ...
      onClick={() => {
        if (setSelectedPokemon) {
          setSelectedPokemon((prev) => togglePokemon(prev, pokemon));
        }
      }}
    >
      <div
        className={`rounded-2xl bg-gradient-to-br from-black/10 via-white/70 to-black/10 `}
      >
